...
import PredictionPanelV2 from '../components/PredictionPanelV2'


const MOCK_WORDS = [
  { word: 'apple', category: 'aliments' },
  { word: 'cat', category: 'animaux' },
  { word: 'dog', category: 'animaux' },
  { word: 'bird', category: 'animaux' },
  { word: 'hammer', category: 'objet' },
  { word: 'bicycle', category: 'objet' },
  { word: 'car', category: 'objet' },
  { word: 'house', category: 'objet' },
  { word: 'tree', category: 'objet' }
]

const CLASSES = ['apple', 'cat', 'dog', 'bird', 'hammer','bicycle','car','house','tree']



export function GamePageV2({ user, onLogout, onGameFinished }) {
  ...
  try {
    const res = await fetch('http://34.196.174.14:8000/predict/base64', {
      method: 'POST',
      headers: {
    ...
    // json = { prediction: number }
    if (typeof json.prediction === 'number') {
      setPredictions([CLASSES[json.prediction]])
      console.log([CLASSES[json.prediction]])
    } else {
      setPredictions([])
    }
